...
if save: plt.savefig("figures/u"+str(id)+".pdf")
plt.show()
...
if save: plt.savefig("figures/uh"+str(id)+".pdf")
plt.show()
###
np.random.seed(3)
ngrid = 12*N
...
for e,(xi1,xi2) in enumerate(zip(X.reshape(-1),Y.reshape(-1))):
    xi = np.array([xi1,xi2])
    loss, gradxi, x = recon.backprop(xi, u, returngrad=False)
    cf[e] = loss
    ...
    g[e]=0.5*np.linalg.norm(matAt.dot(u_hat))**2 - f[e]
    ###
...
plt.yticks(fontsize=fontsize)
plt.tight_layout(pad=0.5)
if save: plt.savefig("figures/g"+str(id)+".pdf")
...
plt.yticks(fontsize=fontsize)
plt.tight_layout(pad=0.5)
if save: plt.savefig("figures/f"+str(id)+".pdf")
...
plt.yticks(fontsize=fontsize)
plt.tight_layout(pad=0.5)
if save: plt.savefig("figures/J"+str(id)+".pdf")
